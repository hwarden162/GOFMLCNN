...
def get_data_loaders(mode, train_transforms, val_transforms):
    cwd = os.getcwd()
    if mode == "large":
        train_dir = os.path.join(cwd, "image_patches", "large", "train")
    if mode == "small":
        train_dir = os.path.join(cwd, "image_patches", "small", "train")
    if mode in ["masked", "mask"]:
        train_dir = os.path.join(cwd, "image_patches", "masked", "train")
...
for mode in modes:
    print (f"Training {mode.title()}")
    train_transforms, val_transforms = get_data_transforms(mode)
    dataloaders = get_data_loaders(mode, train_transforms, val_transforms)
    ...
    print(f"Training complete for {mode.title()}")
    model.load_state_dict(best_model_wts)
    
    save_dir = get_save_dir(mode)
    os.makedirs("saved_models", exist_ok=True)
    torch.save(model.state_dict(), save_dir)
    print(f"Best model saved as {save_dir}")
